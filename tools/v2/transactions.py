...
tool_transactions = Tool(
    ...
    inputSchema={
        ...
        'properties': {
            ...
            'sort': { 'type': 'string', 'enum': ['modification_number', 'action_date', 'federal_action_obligation', 'face_value_loan_guarantee', 'original_loan_subsidy_cost', 'action_type_description', 'description'], 'default': 'action_date' },
...
response_schema = {
    ...
    'properties': {
        'results': {
            ...
            'items': {
                'type': 'object',
                'required': ['action_date', 'action_type', 'action_type_description', 'description', 'face_value_loan_guarantee', 'federal_action_obligation', 'id', 'modification_number', 'original_loan_subsidy_cost', 'type', 'type_description'],
                'properties': {
                    'action_date': { 'type': 'string', 'description': 'Action date in the format YYYY-MM-DD' },
...
def call_tool_transactions(arguments: dict[str, Any]):
    ...
    if award_id == None:
        raise McpError(ErrorData(
            code=INVALID_PARAMS,
            message='award_id is a required argument'
        ))
